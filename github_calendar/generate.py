...
def fetch_issues(repository: str, token: str) -> list[dict]:
    ...
    page = 1
    while True:
        query = urllib.parse.urlencode({"state": "open", "labels": "calendar:event,calendar:approved", "per_page": 100, "page": page})
...
def generate(issues: Iterable[dict], repository: str, output: Path) -> None:
    ...
    for issue in issues:
        ...
        if "calendar:approved" not in labels or not {"calendar:exclude", "calendar:private"}.isdisjoint(labels):
            continue
        try:
