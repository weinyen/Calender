...
次の行
"""
    return {"number": number, "title": title, "body": body, "labels": labels or [{"name": "calendar:event"}, {"name": "calendar:approved"}], "updated_at": "2026-08-25T00:00:00Z"}


class GenerateTests(unittest.TestCase):
    def test_parse_and_render_timed_event(self):
        event = issue_to_event(issue(labels=[{"name": "calendar:event"}, {"name": "calendar:approved"}, {"name": "group:development"}, {"name": "type:meeting"}]), "owner/repo")
    ...
    def test_private_and_excluded_are_omitted_and_groups_created(self):
        issues = [
            issue(1, labels=[{"name": "calendar:event"}, {"name": "calendar:approved"}, {"name": "group:development"}]),
            issue(2, labels=[{"name": "calendar:event"}, {"name": "calendar:approved"}, {"name": "calendar:private"}]),
            issue(3, labels=[{"name": "calendar:event"}, {"name": "calendar:approved"}, {"name": "calendar:exclude"}]),
            issue(4, labels=[{"name": "calendar:event"}]),
        ]
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotIn("github-issue-4", all_calendar)
